...
describe('allocate — FIFO batch consumption', () => {
  it('fully consumes the oldest batch before touching a newer one', () => {
    const lots = [
      lot({ material: 'M1', batch: '0000000100', sourceSpp: 'A', quantity: 5 }),
      lot({ material: 'M1', batch: '0000000200', sourceSpp: 'A', quantity: 20 }),
    ]
    const { fillRows, reconciliation } = allocate(lots, [req('M1', 'B', 12)])
  ...
  it('never fragments more batches than necessary (uses one full batch, not two partial ones)', () => {
    const lots = [
      lot({ material: 'M1', batch: '0000000100', sourceSpp: 'A', quantity: 10 }),
      lot({ material: 'M1', batch: '0000000200', sourceSpp: 'A', quantity: 10 }),
    ]
    const { fillRows } = allocate(lots, [req('M1', 'B', 10)])
...
describe('allocate — same batch number on different source SPP', () => {
  it('treats them as two independent lots, not one merged pool', () => {
    const lots = [
    ...
    const { fillRows, reconciliation } = allocate(lots, [req('M1', 'TARGET', 4)])
    expect(reconciliation[0].status).toBe('ok')
    expect(fillRows).toHaveLength(2)
...
describe('allocate — already at target (no-op transfer)', () => {
  it('does not generate a fill row when stock already sits at the target СПП', () => {
    const lots = [lot({ material: 'M1', batch: '0000000100', sourceSpp: 'TARGET', quantity: 5 })]
    const { fillRows, reconciliation } = allocate(lots, [req('M1', 'TARGET', 5)])
  ...
  it('only consumes the already-at-target lot for the amount actually needed, leaving the rest as surplus', () => {
    const lots = [lot({ material: 'M1', batch: '0000000100', sourceSpp: 'TARGET', quantity: 8 })]
    const { reconciliation, surplusLots } = allocate(lots, [req('M1', 'TARGET', 5)])
    expect(reconciliation[0].alreadyAtTargetQuantity).toBe(5)
    expect(surplusLots).toHaveLength(1)
    expect(surplusLots[0].remainingQuantity).toBe(3)
  })

  it('still draws other lots via transfer when the already-at-target lot is insufficient', () => {
    const lots = [
...
describe('allocate — deficit', () => {
  it('transfers everything available and flags the shortfall as critical', () => {
    const lots = [lot({ material: 'M1', batch: '0000000100', sourceSpp: 'A', quantity: 3 })]
    const { fillRows, reconciliation } = allocate(lots, [req('M1', 'B', 10)])
    expect(fillRows).toHaveLength(1)
    expect(fillRows[0].quantity).toBe(3)
  ...
  it('flags a total absence of stock as critical with zero coverage', () => {
    const { fillRows, reconciliation } = allocate([], [req('GHOST', 'B', 4)])
    expect(fillRows).toHaveLength(0)
    expect(reconciliation[0].status).toBe('deficit')
...
describe('allocate — shared pool across requirements', () => {
  it('reduces availability for a later requirement on the same material', () => {
    const lots = [lot({ material: 'M1', batch: '0000000100', sourceSpp: 'A', quantity: 10 })]
    const requirements = [req('M1', 'X', 7), req('M1', 'Y', 5)]
    const { reconciliation } = allocate(lots, requirements)
    expect(reconciliation[0].status).toBe('ok')
  ...
  it('leaves unused stock as surplus once every requirement is processed', () => {
    const lots = [lot({ material: 'M1', batch: '0000000100', sourceSpp: 'A', quantity: 10 })]
    const { surplusLots } = allocate(lots, [req('M1', 'X', 4)])
    expect(surplusLots).toHaveLength(1)
